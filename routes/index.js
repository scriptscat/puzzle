...
router.all("/", async function (req, res, next) {
  ...
  if (
    puzzle.beforeRsp instanceof Function &&
    (await puzzle.beforeRsp(req, res))
  )
    return;
...
router.post("/u/:username", async function (req, res, next) {
  try {
    ...
    let error = "";
    try {
      req.session.user = await user.update(
        Object.assign({ username: user.user.username }, req.body),
        true
      );
    } catch (err) {
      error = err.message;
